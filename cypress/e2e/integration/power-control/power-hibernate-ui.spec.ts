  ...
  cy.request({
    ...
    return statusLogChain
      ...
      .then(() => {
        ...
        // Wake from Hibernate (S4) requires PowerUp (2) — an OOB AMT command that
        // cold-powers the device on, triggering Windows to restore from hiberfil.sys.
        // OsToFullPower (500) is an in-band IPS call and cannot reach a suspended OS.
        const wakeAction = PowerActions.PowerUp
        cy.task('log', `  ${phaseLabel}: state is not On — sending wake action ${wakeAction} (PowerUp OOB) …`)
        cy.request({
          method: 'POST',
          ...
          const wakeLogChain = wakeRes.status !== httpCodes.SUCCESS
            ? cy.task('log', `  ❌ Assertion failed: ${phaseLabel}: wake action ${wakeAction} must return HTTP 200 — got ${wakeRes.status}`)
            : cy.wrap(null, { log: false })

          return wakeLogChain
            .then(() => { expect(wakeRes.status, `${phaseLabel}: wake action ${wakeAction} must return HTTP 200`).to.eq(httpCodes.SUCCESS) })
            .then(() => {
              cy.task('log', `  ${phaseLabel}: wake action ${wakeAction} → HTTP ${wakeRes.status}; waiting ${waitAfterPowerUpMs / 1000}s to re-check …`)
                ...
                return reCheckStatusLogChain
                  ...
                  .then(() => {
                    if (newState !== PowerStateValues.On) {
                      return cy.task('log', `  ❌ Assertion failed: ${phaseLabel}: device must be On (${PowerStateValues.On}) after wake — got ${newState} (${newLabel})`)
                    }
                  ...
                  .then(() => {
                    expect(
                      newState,
                      `${phaseLabel}: device must be On (${PowerStateValues.On}) after wake — got ${newState} (${newLabel})`
...
describe('TC_POWER_ACTION_HIBERNATE - Hibernate via Web UI dropdown (isolated run)', () => {
  ...
  afterEach(function () {
    ...
    ensureDevicePoweredOn('Cleanup', 'Cleanup complete: device is On', 60000, 60000)
    cy.wait(5000)
  })

  // ── TC_POWER_ACTION_HIBERNATE ───────────────────────────────────────────────
  it(
    'TC_POWER_ACTION_HIBERNATE: login to Web UI → click Hibernate in dropdown → wait 2 min → verify powerstate via API → wake to On',
    function () {
      ...
      // Color mapping: green → On | yellow → Sleep | red → Off/Hibernate/other.
      ...
      // before searching for "Hibernate" text.
      //
      ...
      cy.task('log', `  ✅ "Hibernate" clicked — AMT requesting OS hibernate (action ${PowerActions.Hibernate}) — OS will save RAM to disk`)
      ...
      // Expect Hibernate (S4) = powerstate 7.
      cy.task('log', `  Step 6: Checking AMT power state via API (expecting Hibernate / S4 = ${PowerStateValues.Hibernate}) …`)
      cy.request({
        method: 'GET',
        ...
        return statusLogChain
          ...
          .then(() => {
            if (state !== PowerStateValues.Hibernate) {
              return cy.task('log', `  ❌ Assertion failed: powerstate must be ${PowerStateValues.Hibernate} (Hibernate / S4) — got ${state} (${label})`)
            }
            return cy.wrap(undefined, { log: false })
          })
          .then(() => {
      ...
      // Hibernate is not On or Sleep, so the toolbar renders the icon as red.
