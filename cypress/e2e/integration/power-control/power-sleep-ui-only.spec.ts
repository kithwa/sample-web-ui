 ...
 *   4. Click "Sleep"
 *   5. Wait 3 minutes for the OS to process the ACPI signal
...
describe('TC_POWER_ACTION_SLEEP - Sleep via Web UI dropdown (isolated run)', () => {
  ...
  it(
    'TC_POWER_ACTION_SLEEP: login to Web UI → click Sleep in dropdown → wait 3 min → verify powerstate via API → wake to On',
    function () {
      ...
      // Refresh the mode_standby icon first so the UI reflects the latest AMT
      // state from the pre-test API check, then assert green.
      ...
      // before searching for "Sleep" text.
      //
      ...
      cy.request({
        ...
        return statusLogChain
          ...
          .then(() => {
            // Only accept Deep Sleep S3 (PowerState value == 4) as pass
            if (state !== PowerStateValues.SleepDeep) {
              return cy.task('log', `  ❌ Assertion failed: powerstate value must be 4 (Sleep Deep / S3) — got ${state} (${label})`)
            }
            return cy.wrap(undefined, { log: false })
          })
          .then(() => {
            expect(
              state,
              `powerstate value must be 4 (Sleep Deep / S3) — got ${state} (${label})`
            ).to.eq(PowerStateValues.SleepDeep)
          })
