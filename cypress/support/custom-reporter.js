...
class CustomTabularReporter extends mocha.reporters.Spec {
  ...
  generateReport(data, options) {
    ...
    const reportDir = options.reporterOptions?.reportDir || 'cypress/reports'
    const reportName = options.reporterOptions?.reportName || `test-report-${timestamp}`
    ...
    fs.writeFileSync(path.join(reportDir, `${reportName}.txt`), textReport)

    // Generate CSV report
    const csvReport = this.generateCSVReport(data)
    fs.writeFileSync(path.join(reportDir, `${reportName}.csv`), csvReport)

    // Generate JSON report
    fs.writeFileSync(path.join(reportDir, `${reportName}.json`), JSON.stringify(data, null, 2))

    // Generate HTML report
    const htmlReport = this.generateHTMLReport(data)
    fs.writeFileSync(path.join(reportDir, `${reportName}.html`), htmlReport)

    console.log('\n' + '='.repeat(100))
    console.log('📊 TEST REPORT GENERATED:')
    console.log(`   - Text Report: ${path.join(reportDir, `${reportName}.txt`)}`)
    console.log(`   - CSV Report:  ${path.join(reportDir, `${reportName}.csv`)}`)
    console.log(`   - JSON Report: ${path.join(reportDir, `${reportName}.json`)}`)
    console.log(`   - HTML Report: ${path.join(reportDir, `${reportName}.html`)}`)
